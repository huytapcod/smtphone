...
public class AuthenticationService {
    ...
    @NonFinal
    protected final String SIGNER_KEY = "Z9rBKQTObcpiISqM+Q5hfqT6zXFuuOC6MhqwMcKx/IprAqt7U8QXrzgFmJkzTBOz";
    ...
    public AuthenticationResponse authenticate(AuthenticationRequest authenticationRequest) {
        var user = userRepository.findByName(authenticationRequest.getName())
                .orElseThrow(()-> new AppException(Errorcode.USER_NOT_EXISTED));

        PasswordEncoder passwordEncoder = new BCryptPasswordEncoder(10);
        boolean athenticated = passwordEncoder.matches(authenticationRequest.getPassword(),
                user.getPassword());
        if (!athenticated)
